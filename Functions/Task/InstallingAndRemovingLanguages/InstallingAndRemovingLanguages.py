...
def SearchUninstallation():
    command = 'schtasks /Query /TN \"Microsoft\\Windows\\LanguageComponentsInstaller\\Uninstallation"'
    ...
    # Использование регулярного выражения для поиска строки с задачей и её статусом
    match = re.search(r'Uninstallation\s+.*\s+(Ready|Running|Disabled)', output)
    if match:
        # Получение статуса задачи из найденной строки
        status = match.group(1)
        if status == "Disabled":
