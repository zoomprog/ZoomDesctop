...
def ValueEmailOn():
    try:
        ...
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\email", 0, winreg.KEY_WRITE)
        # Установить значение
...
def SearchValueEmail():
    try:
        # Открыть ключ реестра в режиме чтения
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\email", 0, winreg.KEY_READ)
        # Прочитать значение
...
def LetAppsAccessEmailOn():
    try:
        ...
        # Удалить значение
        winreg.DeleteValue(key, "LetAppsAccessEmail")
        # Закрыть ключ
        winreg.CloseKey(key)
...
def LetAppsAccessEmailOff():
    try:
        # Открыть ключ реестра в режиме записи
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\AppPrivacy", 0, winreg.KEY_WRITE)
        # Установить значение
        winreg.SetValueEx(key, "LetAppsAccessEmail", 0, winreg.REG_DWORD, 2)
        # Закрыть ключ
        winreg.CloseKey(key)
    except Exception as e:
        print(f"Ошибка при установке значения: {e}")
