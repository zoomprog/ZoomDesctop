...
def ValueAATMDOn():
    try:
        ...
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\activity", 0, winreg.KEY_WRITE)
        # Установить значение
...
def SearchValueAATMD():
    try:
        # Открыть ключ реестра в режиме чтения
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\activity", 0, winreg.KEY_READ)
        # Прочитать значение
...
def LetAppsAccessMotionOn():
    try:
        ...
        # Удалить значение
        winreg.DeleteValue(key, "LetAppsAccessMotion")
        # Закрыть ключ
        winreg.CloseKey(key)
...
def LetAppsAccessMotionOff():
    try:
        ...
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\AppPrivacy", 0, winreg.KEY_WRITE)
        # Установить значение
        winreg.SetValueEx(key, "LetAppsAccessMotion", 0, winreg.REG_DWORD, 2)
        # Закрыть ключ
        winreg.CloseKey(key)
    except Exception as e:
        print(f"Ошибка при установке значения: {e}")
...
def SearchLetAppsAccessMotion():
    try:
        ...
        # Прочитать значение
        value, reg_type = winreg.QueryValueEx(key, "LetAppsAccessMotion")
        # Закрыть ключ
        winreg.CloseKey(key)
        if value == 2:
            return 'Disabled'
