...
def ValueAATIOn():
    try:
        ...
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\userAccountInformation", 0, winreg.KEY_WRITE)
        # Установить значение
...
def SearchValueAATI():
    try:
        ...
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\userAccountInformation", 0, winreg.KEY_READ)
        # Прочитать значение
...
def LetAppsAccessAccountInfoOn():
    try:
        ...
        # Удалить значение
        winreg.DeleteValue(key, "LetAppsAccessAccountInfo")
        # Закрыть ключ
        winreg.CloseKey(key)
...
def LetAppsAccessAccountInfoOff():
    try:
        # Открыть ключ реестра в режиме записи
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\AppPrivacy", 0, winreg.KEY_WRITE)
        # Установить значение
        winreg.SetValueEx(key, "LetAppsAccessAccountInfo", 0, winreg.REG_DWORD, 2)
        # Закрыть ключ
        winreg.CloseKey(key)
    except Exception as e:
        print(f"Ошибка при установке значения: {e}")
